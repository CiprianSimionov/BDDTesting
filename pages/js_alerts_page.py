...
class JsAlertsPage(BasePage):
    ...
    def jsconfirm_clicked_ok(self):
        result = self.chrome.find_element(*self.result_text)
        assert result.text == "You clicked: Ok"

    def click_cancel_button_jsconfirm(self):
        self.chrome.switch_to.alert.dismiss()

    def jsconfirm_clicked_cancel(self):
        result = self.chrome.find_element(*self.result_text)
    ...
    def send_text_to_confirm(self, text):
    ...
    def you_entered_null_on_confirm(self):
        result = self.chrome.find_element(*self.result_text)
